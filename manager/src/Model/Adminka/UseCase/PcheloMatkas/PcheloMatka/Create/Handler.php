...
class Handler
{
    ...
    public function handle(Command $command): void
    {
        ...
        $rasa = $this->rasas->get(new RasaId($command->ras));
//        dd($rasaPchel->getName());
        ...
        $rasaName = $rasa->getName();

        if ($rasaName == " -- нет нужной" and ($kategoria->getName() == "бр" || $kategoria->getName() == "бр/а" || $kategoria->getName() == "бр/с")) {
        ...
        $command->name = $rasaName . "-" . $command->sort. " : " . $kategoria->getName()  . " : " . $mesto->getNomer() . "_пн-" . $persona->getNomer() . " : " . $command->title;
        $korotkoName = $rasaName . "-" . $command->sort. " : ". $command->title;
