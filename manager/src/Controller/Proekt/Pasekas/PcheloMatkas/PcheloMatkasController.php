...
class PcheloMatkasController extends AbstractController
{
     ...
     * @param PcheloMatkaRepository $repo
     * @return Response
     */
    public function index(Request $request, PcheloMatkaFetcher $fetcher, PcheloMatkaRepository $repo): Response
    {
        $uchastie = $repo->hasForUchast($this->getUser()->getId());

        if (!$uchastie){
            $this->addFlash('error', 'Внимание!!! В списке нет данных о ваших матках.  Посмотрите  эту страницу и  зарегистрируйте Пчело - Матку!  ');
            return $this->redirectToRoute('app.proekts.page_glavas.pchelomatka.show');
        }
